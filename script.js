...
async function createDoc() {
  ...
  try {
    ...
    const report = {
      ...
      placesCovered:
        formData.get('places_covered') || '',

      datesCovered:
        formData.get('dates_covered') || '',

      participants:
        formData.get('participants') || '',

      narratives:
        formData.getAll('narrative[]'),
    ...
    alert(
      'After Activity Report generated successfully!'
    );
    ...
    console.error(
      'AAR generation error:',
      error
    );


    alert(
      'Unable to generate the After Activity Report.\n\n' +
      error.message
    );
    ...
    button.textContent =
      'GENERATE AFTER ACTIVITY REPORT';
